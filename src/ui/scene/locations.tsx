...
function DrivewayScene({ loc }: { loc: LocationDef }) {
  return (
    <g>
      ...
      <House x={7.6} y={1.5} w={1.7} d={1.4} h={34} rh={16} wall="#e6dccb" roof="#8b7fae" />
      <Tree x={3.4} y={3.0} />
      ...
      <House x={6.85} y={6.8} w={1.6} d={1.3} h={30} rh={14} wall="#f0e9da" roof="#9db98a" />
  ...
  return (
    <g>
      ...
      <Box x={2.9} y={1.9} w={1.6} d={1.4} h={38} top="#d8cdb8" right="#e0d5c0" front="#e8ddc8">
        <Awning a={[2.9, 3.3]} b={[4.5, 3.3]} top={26} drop={8} color="#8b7fae" />
      </Box>
      {/* mural wall */}
      <Box x={6.6} y={2.0} w={2.4} d={1.2} h={40} top="#c4beb0" right="#d8d2c4" front="#e8b4c8" />
      <circle cx={iso(8.3, 3.2)[0]} cy={iso(8.3, 3.2)[1] - 22} r="9" fill="#7fb4c9" stroke={INK} strokeWidth="1.1" />
      <circle cx={iso(8.9, 3.2)[0]} cy={iso(8.9, 3.2)[1] - 14} r="6" fill="#e6c86e" stroke={INK} strokeWidth="1.1" />
...
function VeniceScene({ loc }: { loc: LocationDef }) {
  return (
    <g>
      <polygon points={poly([iso(-2, -2), iso(12, -2), iso(12, 0.8), iso(-2, 0.8)])} fill="#7fb4c9" stroke="none" />
      <path
        ...
        opacity="0.7"
      />
      <polygon points={poly([iso(-2, 0.8), iso(12, 0.8), iso(12, 5.2), iso(-2, 5.2)])} fill="#e6d7ae" stroke="none" />
      {/* towels */}
      <polygon points={poly([iso(1.8, 2.2), iso(2.5, 2.2), iso(2.5, 2.55), iso(1.8, 2.55)])} fill="#e8b4c8" stroke={INK} strokeWidth="1" />
      <polygon points={poly([iso(6.9, 1.7), iso(7.6, 1.7), iso(7.6, 2.05), iso(6.9, 2.05)])} fill="#7fb4c9" stroke={INK} strokeWidth="1" />
      <Palm x={1.0} y={3.6} />
      <Palm x={5.3} y={3.1} s={0.85} />
      <Palm x={9.8} y={3.7} s={1.05} />
      {/* boardwalk */}
      <polygon points={poly([iso(-2, 5.2), iso(12, 5.2), iso(12, 7.2), iso(-2, 7.2)])} fill="#d9b98a" stroke={INK} strokeWidth="1.2" />
      {Array.from({ length: 28 }, (_, i) => -2 + i * 0.5).map((gx) => {
        const [ax, ay] = iso(gx, 5.2);
        const [bx, by] = iso(gx, 7.2);
        return <line key={gx} x1={ax} y1={ay} x2={bx} y2={by} stroke="#c9a877" strokeWidth="1.3" />;
      })}
...
function SantamonicaScene({ loc }: { loc: LocationDef }) {
  ...
  return (
    <g>
      <polygon points={poly([iso(-2, -2), iso(12, -2), iso(12, 1.4), iso(-2, 1.4)])} fill="#7fb4c9" stroke="none" />
      ...
      <polygon points={poly([iso(-2, 1.4), iso(12, 1.4), iso(12, 2.4), iso(-2, 2.4)])} fill="#e6d7ae" stroke="none" />
      {/* storefront row */}
      <Box x={0.6} y={2.5} w={1.8} d={1.2} h={40} top="#f0e9da" right="#e6dfd0" front="#efe8da">
        <Awning a={[0.6, 3.7]} b={[2.4, 3.7]} top={28} drop={8} color="#e8b4c8" />
      </Box>
...
function BeverlyhillsScene({ loc }: { loc: LocationDef }) {
  return (
    <g>
      {[
        { x: 0.5, w: 1.7, awn: '#e8b4c8' },
        { x: 2.5, w: 1.5, awn: loc.sceneColors.accent },
        { x: 6.4, w: 1.7, awn: '#7fb4c9' },
        { x: 8.4, w: 1.5, awn: '#e6c86e' },
      ].map((b, i) => (
...
function PalisadesScene({ loc }: { loc: LocationDef }) {
  return (
    <g>
      ...
      <polygon points={poly([iso(-2, -2), iso(12, -2), iso(12, -0.6), iso(-2, -0.6)])} fill="#7fb4c9" stroke="none" />
      <polygon
        points={poly([
          iso(-2, -0.6),
          iso(1, -0.9),
      ...
      <Box x={0.7} y={1.6} w={1.8} d={1.3} h={38} top="#f5f0e4" right="#efe8da" front="#f7f2e8">
        <Awning a={[0.7, 2.9]} b={[2.5, 2.9]} top={26} drop={8} color="#9db98a" />
      </Box>
